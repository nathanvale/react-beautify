...
const supported_languages =  ["javascript", "javascriptreact", "typescript", "typescriptreact","css"];

export function activate(context : ExtensionContext) {
    context
        .subscriptions
        .push(commands.registerCommand("react.beautify", () => {
...
function registerFormatter(context : ExtensionContext, languageId : string) {
    context
        .subscriptions
        .push(languages.registerDocumentFormattingEditProvider(languageId, {
    ...
    context
        .subscriptions
        .push(languages.registerDocumentRangeFormattingEditProvider(languageId, {
            provideDocumentRangeFormattingEdits: (document, range, options, token) => {
                let begin = new Position(range.start.line, 0);
                let end = range
                    .end
                    .translate(0, Number.MAX_VALUE);
...
function onWillSave(event : TextDocumentWillSaveEvent) {
    ...
    if (supports(doc.languageId) && getConfig("onSave", false)) {
        ...
        let editor = window
            .visibleTextEditors
            .find(ed => ed.document && ed.document.fileName === doc.fileName);
        let options = editor
            ? editor.options
            : workspace.getConfiguration('editor');
        event.waitUntil(format(doc, r, options).then(txt => {
...
export function format(doc : TextDocument, range : Range, defaults : any) : Promise < string > {
    if(doc) {
        ...
        if (langId && supports(langId)) {
            ...
            return loadOptions(root, defaults).then(options => {
                let t = getConfig < string > ("formatter") || "prettydiff";
                let path;
                let prettydiff2path
                if (t === "prettydiff2") {
                    prettydiff2path = getConfig < string > ("prettydiff2.path");
                    if (!t) {
                        return Promise.reject < string > (`Pretty diff 2 path not configured`);
                    }
                    path = resolve(root, prettydiff2path);
                } else {
                    path = resolve(root, t);
                }
                return [
                    options, formatters.make(path, t, langId)
                ];
            }).then(optFmt => {
...
function loadOptions(root : string, defaults : any) : Promise < any > {
    if(root) {
        let relpath = getConfig < string > ("configFilePath") || ".jsbeautifyrc";
